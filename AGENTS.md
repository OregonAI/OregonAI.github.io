# AGENTS.md — OregonAI.github.io

The organization landing page. **Not a corpus** — it holds no documents, and every fact
about a corpus is read live from that corpus at build time.

## Rules

- `site/` is generated by `build_site.py` and gitignored. Never hand-edit it; edit the
  builder.
- Counts come from each corpus's published `corpus-index.json`. A corpus that publishes
  none renders with its declared status and **no counts** — the page degrades to the
  registry rather than inventing numbers. Keep it that way.
- `status` in `CORPORA` is a **fallback**, shown only when a corpus publishes no index.
  A corpus that does publish one is reported live regardless of what the registry says.
- **Never publish an MCP URL that does not answer.** A reader cannot distinguish "not
  deployed" from "deployed and broken", so `mcp=None` until the endpoint actually
  responds. Same for repository links — those are probed, not assumed.
- Descriptions must match what is actually ingested. A blurb saying "not yet started"
  beside a live document count is worse than no blurb.
